...
- [Development guidelines](#development-guidelines)
    - [General MAGNUS](#general-magnus-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MAGNUS uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
MAGNUS is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/magnus-project/magnus/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_magnus
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_magnus --log_level=test_suite
$ valgrind -v --leak-check=full src/magnusd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_magnus.coverage/index.html`.
```

**Sanitizers**
MAGNUS can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates MAGs (if wallet is enabled).
...
However, in open source software such as MAGNUS, where everyone uses
...
pay attention to for reviewers of MAGNUS code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MAGNUS uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MAGNUS so that they can be integrated
...
file descriptors it will cause problems with MAGNUS's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:MAGNUS-Project/MAGNUS.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `magnus-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `magnus-cli` and the GUI debug console use this table to determine how to
