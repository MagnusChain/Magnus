...
*Setup instructions for a gitian build of MAGNUS using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MAGNUS
...
to the MAGNUS GitHub release page.
...
- [Building MAGNUS](#building-magnus-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/magnus-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/magnus-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/magnus-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/magnus-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/magnus-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/magnus-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/magnus-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@magnus.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/magnus-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/magnus-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/magnus-osx-*-build.assert
```
