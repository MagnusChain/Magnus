...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/MAGNUSParams"
    else
        PARAMS_DIR="$HOME/.magnus-params"
    fi
fi
...
Failed to fetch the MAGNUS zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
MAGNUS - fetch-params.sh

This script will fetch the MAGNUS zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common MAGNUS zkSNARK parameters. Note that it is
