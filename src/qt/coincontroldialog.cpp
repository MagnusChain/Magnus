...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select MAG Outputs to Spend" :
            "Select Shielded MAG to Spend");
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show MAGNUS address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
