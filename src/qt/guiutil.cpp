...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZmag)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZmag)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZmag);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZmag)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZmag)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZmag);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter MAGNUS address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no MAGNUS: URI
...
{
    // Convert magnus:// to magnus:
    //
    //    Cannot handle this later, because magnus:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", MAGNUS_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MAGNUS (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MAGNUS (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "MAGNUS.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MAGNUS*.lnk
...
{
    return GetAutostartDir() / "magnus.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a magnus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=MAGNUS (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=MAGNUS (regtest)\n";
        else
            optionFile << "Name=MAGNUS\n";
