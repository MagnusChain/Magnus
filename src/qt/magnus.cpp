...
#include "qt/magnus/splash.h"
#include "qt/magnus/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/magnus/governancemodel.h"
#include "qt/magnus/mnmodel.h"
...
{
    return QCoreApplication::translate("magnus-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in magnus.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in magnus.qrc)
...
/** Class encapsulating MAGNUS startup and shutdown.
...
/** Main MAGNUS application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (MAGNUSGUI)
...
{
    window = new MAGNUSGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MAGNUSGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MAGNUSGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(MAGNUSGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MAGNUSGUI::DEFAULT_WALLET);
        ...
        // MAGNUS: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MAGNUSGUI::handlePaymentRequest);
        connect(window, &MAGNUSGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. MAGNUS can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(magnus_locale);
    Q_INIT_RESOURCE(magnus);
    ...
    /// 6. Determine availability of data and blocks directory and parse magnus.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", MAGNUS_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // magnus: links repeatedly have their payment requests routed to this process:
