...
#include "qt/magnus/forms/ui_snackbar.h"
#include "qt/magnus/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MAGNUSGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MAGNUSGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
