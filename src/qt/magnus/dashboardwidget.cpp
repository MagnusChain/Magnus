...
#include "qt/magnus/dashboardwidget.h"
#include "qt/magnus/forms/ui_dashboardwidget.h"
#include "qt/magnus/sendconfirmdialog.h"
#include "qt/magnus/txrow.h"
...
DashboardWidget::DashboardWidget(MAGNUSGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareMag, "square-chart-mag");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelMag, "text-chart-mag");
    ...
    setCssProperty(ui->labelAmountMag, "text-stake-mag-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MAGNUSGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isMag = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MAG, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            mag = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalMag += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesMag.append(mag);
        chartData->valuesMN.append(mn);

        int max = std::max(mag, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesMag);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMag > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountMag, "text-stake-mag");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountMag, "text-stake-mag-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountMag, ui->labelAmountMN});
    ui->labelAmountMag->setText(GUIUtil::formatBalance(chartData->totalMag, nDisplayUnit));
