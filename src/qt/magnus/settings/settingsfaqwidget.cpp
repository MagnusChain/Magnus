...
#include "config/magnus-config.h"
#endif

#include "qt/magnus/settings/settingsfaqwidget.h"
#include "qt/magnus/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(MAGNUSGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableMAG,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableMAG,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("MAGNUS is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. MAGNUS incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("MAGNUS utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of MAGNUS to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableMAGContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received MAGNUS requires 6 confirmations on the network "
               "to become eligible for spending which can take 5-10 minutes.")) +
        formatFAQParagraph(
            tr("Your MAGNUS wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableMAG->setText(unspendableMAGContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of MAGNUS with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("magnus-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #A30000' href='https://discord.com/invite/PbSy5UNtAz'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getMNCollateralRequiredAmount(), BitcoinUnits::MAG)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in MAGNUS Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the MAGNUS network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableMAG, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableMAG);});
    ...
    if (parent)
        connect(parent, &MAGNUSGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
