...
#include "qt/magnus/topbar.h"
#include "qt/magnus/forms/ui_topbar.h"
#include "qt/magnus/lockunlock.h"
#include "qt/magnus/qtutils.h"
#include "qt/magnus/receivedialog.h"
#include "qt/magnus/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/magnus/balancebubble.h"
...
TopBar::TopBar(MAGNUSGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountMag}, "amount-topbar");
    setCssProperty({ui->labelPendingMag, ui->labelImmatureMag, ui->labelAvailableMag, ui->labelLockedMag, ui->labelCollateralMag,
                       ui->labelNextCollateralBlocks, ui->labelMNRewardvalue, ui->labelStakingRewardvalue},
        "amount-small-topbar");
   ...
   /* ui->pushButtonColdStaking->setButtonClassStyle("cssClass", "btn-check-cold-staking-inactive");
    ...
    ui->pushButtonConf->setButtonText("magnus.conf");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MAGNUS")
    updateDisplayUnit();
...
void TopBar::refreshMasternodeStatus()
{
    ...
    if (chainActive.Tip()) {
        ...
        /*std::string currency = std::to_string(p.first);
        int decimal = currency.length();
        if (decimal > 3) {
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralMag->setText(GUIUtil::formatBalance(CMasternode::GetMNCollateral(chainActive.Tip()->nHeight) * COIN, nDisplayUnit));
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // MAG Total
    QString totalMag = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // Expanded
    ui->labelAmountMag->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailableMag->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingMag->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureMag->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedMag->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
