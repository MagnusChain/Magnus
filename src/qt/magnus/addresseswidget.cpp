...
#include "qt/magnus/forms/ui_addresseswidget.h"
#include "qt/magnus/addressfilterproxymodel.h"
#include "qt/magnus/addresslabelrow.h"
#include "qt/magnus/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(MAGNUSGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto magAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(magAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(magAdd);
        if (walletModel->updateAddressBookLabels(magAdd, label.toUtf8().constData(),
