...
#include "qt/magnus/send.h"
#include "qt/magnus/forms/ui_send.h"
#include "qt/magnus/addnewcontactdialog.h"
#include "qt/magnus/qtutils.h"
#include "qt/magnus/sendchangeaddressdialog.h"
#include "qt/magnus/optionbutton.h"
#include "qt/magnus/sendconfirmdialog.h"
#include "qt/magnus/guitransactionsutils.h"
...
SendWidget::SendWidget(MAGNUSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a MAGNUS URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-mag");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this]() { onMAGSelected(true); });
    connect(ui->pushRight, &QPushButton::clicked, [this]() { onMAGSelected(false); });
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent MAGs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield MAGs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent MAGs to shield."));
    }
}
...
void SendWidget::onMAGSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto magAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(magAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(magAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(magAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
