...
const char * const MAGNUS_CONF_FILENAME = "magnus.conf";
const char * const MAGNUS_PID_FILENAME = "magnus.pid";
const char * const MAGNUS_MASTERNODE_CONF_FILENAME = "masternode.conf";


// MAGNUS only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "magnusd -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MAGNUS
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MAGNUS
// Mac: ~/Library/Application Support/MAGNUS
// Unix: ~/.magnus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Magnus";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MAGNUSParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MAGNUSParams
    // Mac: ~/Library/Application Support/MAGNUSParams
    // Unix: ~/.magnus-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MagnusParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "MagnusParams";
#else
    // Unix
    return pathRet / ".magnus-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/magnus/sapling-spend.params";
        sapling_output = "/usr/share/magnus/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/magnus/sapling-spend.params";
            sapling_output = "/usr/local/share/magnus/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", MAGNUS_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", MAGNUS_PID_FILENAME));
