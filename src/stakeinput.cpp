...
CMagStake* CMagStake::NewMagStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CMagStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CMagStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CMagStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CMagStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CMagStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CMagStake::CreateTxOuts(const CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal) const
{
...
CDataStream CMagStake::GetUniqueness() const
{
...
const CBlockIndex* CMagStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CMagStake: uninitialized pindexFrom");
    return pindexFrom;
}
