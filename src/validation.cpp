...
#if defined(NDEBUG)
#error "MAGNUS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umag) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k MAG
    ...
    if (nHeight > 0 && nHeight < nLast) {
        return 400 * COIN;
    } else if (nHeight >= nLast && nHeight <= 500) {
        return 0.25 * COIN;
        ...
        return 5 * COIN;
    } else if (nHeight > 750000 && nHeight <= 10542518) {
        return 10 * COIN;
    } else if (nHeight == 10542519) {
        return 7.50 * COIN;
    } else {
        return 0 * COIN;
    }
...
{
    util::ThreadRename("magnus-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Magnus Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
