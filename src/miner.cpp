...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("MAGNUSMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("MAGNUSMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("MAGNUSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("magnus-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MAGNUSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("MAGNUSMiner exception");
    } catch (...) {
        LogPrintf("MAGNUSMiner exception");
    }

    LogPrintf("MAGNUSMiner exiting\n");
}
