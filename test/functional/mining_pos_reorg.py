...
class ReorgStakeTest(MagnusTestFramework):
    ...
    def check_money_supply(self, expected_mag):
        # verify that nodes have the expected MAG supply
        mag_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(mag_supply, [DecimalAmt(expected_mag)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that MAG supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check MAG supply...")
